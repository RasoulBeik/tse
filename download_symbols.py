...
def download_symbols(symbols=None):
    ...
    failed_symbols = rep.download_ohlc_daily_for_symbols(symbols, do_parallel=True)
    # failed_symbols = rep.generate_periodic_data(symbol_names=symbols, timeframes=['W', 'WJ'])
    # failed_symbols = rep.generate_periodic_data(symbol_names=symbols, timeframes=['M', 'MJ'])
    print('End time:', datetime.now().time())
    
if __name__ == "__main__":
    download_symbols(symbols)

    # info_df = SymbolRepository.download_base_info_for_symbols(symbol_names=symbols)
    # print(info_df)